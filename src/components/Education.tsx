      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-2xl font-bold text-gray-800 dark:text-white transition-colors duration-300">{t('education.formal')}</h3>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {education.map((edu, index) => {
              ...
              return (
                <div key={index} className={`bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-500 border-l-4 ${colorClasses.border} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} group`} style={{transitionDelay: `${index * 200}ms`}}>
                  ...
                  <h4 className="text-xl font-bold text-gray-800 dark:text-white mb-2 group-hover:text-blue-600 transition-colors duration-300">{edu.degree}</h4>
                  ...
                  <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300 mb-4 transition-colors duration-300">
                    <Calendar size={16} />
                    <span className="font-medium">{edu.period}</span>
                  </div>
                  
                  <p className="text-gray-600 dark:text-gray-300 leading-relaxed transition-colors duration-300">{edu.description}</p>
                </div>
              );
            })}
          </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-2xl font-bold text-gray-800 dark:text-white transition-colors duration-300">{t('education.certifications')}</h3>
          </div>
          
          <div className="grid lg:grid-cols-3 gap-8">
            {certifications.map((cert, index) => (
              <div key={index} className={`bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-500 border border-gray-100 dark:border-gray-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} group`} style={{transitionDelay: `${(index + 2) * 200}ms`}}>
                <div className="flex items-start gap-3 mb-4">
                  <div className="bg-purple-100 dark:bg-purple-900 rounded-full p-2 flex-shrink-0 group-hover:bg-purple-200 dark:group-hover:bg-purple-800 transition-colors duration-300">
                    <BookOpen className="text-purple-600 group-hover:scale-110 transition-transform duration-300" size={20} />
                  </div>
                  <div>
                    <h4 className="text-lg font-bold text-gray-800 dark:text-white mb-1 group-hover:text-purple-600 transition-colors duration-300">{cert.title}</h4>
                    {cert.provider && (
                      <p className="text-blue-600 font-semibold mb-2 group-hover:text-purple-500 transition-colors duration-300">{cert.provider}</p>
                    )}
                    <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 mb-3 transition-colors duration-300">
                      <Calendar size={14} />
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm transition-colors duration-300">{cert.description}</p>
                
                <div className="flex flex-wrap gap-2">
                  {cert.skills.map((skill, skillIndex) => (
                    <span 
                      key={skillIndex}
                      className="px-3 py-1 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-full text-xs font-medium hover:bg-purple-100 dark:hover:bg-purple-800/50 hover:scale-105 transition-all duration-300 cursor-pointer"
                    >
                      {skill}
                    </span>
                  ))}
